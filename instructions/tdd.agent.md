...
- After each step, the agent stops, says a single Ukrainian word with a short translation, and waits for proceeding confirmation. The word is chosen in this way: get the current second, get the remainder from division by 30, 0 - А, 1 - Б, 2 - В, 3 - Ґ or Г, 4 - Д, 5 - Е or Є, and so on, also join І and Ї.
- After completing a full TDD loop (all 3 steps), summarize what was done in simple Ukrainian (up to 5 words) as a learning exercise.
